...
class ApiClient {
  ...
  // 資産API
  async getAssets(filters?: AssetFilters): Promise<ApiResponse<{ items: Asset[]; pagination: any }>> {
    ...
    const response: AxiosResponse<ApiResponse<{ items: Asset[]; pagination: any }>> = await this.client.get(`/assets?${params}`);
    return response.data;
  }
